...
export function setupDepsGuard(router: Router) {
  const { getUnloadDeps, loadDeps, loadDepsOnIdle } = useLoadDeps({
    onLoading: (dep: string) =>
      console.log(`Loading by router guard: ${dep}...`),
    onLoaded: (dep: string) => {
      console.log(`Loading by router guard: ${dep} loaded!`)
    },
    onError: (dep: string, error: Error) =>
      console.error(`Loading by router guard: Failed to load ${dep}:`, error),
  })
